...
export function useUpdateClientObligationProfile(clientId: number) {
  ...
  return useMutation({
    mutationFn: async (data: { obligation_types: number[] }) => {
      const response = await apiClient.put<ClientObligationProfile>(
        `/accounting/api/v1/clients/${clientId}/obligation-profile/`,
        data
      );
      return response.data;
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [CLIENT_OBLIGATION_PROFILE_KEY, clientId] });
    },
  });
}
